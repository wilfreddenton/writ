...
**Read this first — version reconciliation is a hard gate.** The API-verification specs were checked against a local Linebender *workspace clone* whose surface differs from the crates.io releases the hard constraint requires:
- `LayoutContext::ranged_builder(fcx, text, scale, quantize: bool)` in the clone has a 5th `quantize` param that **published parley 0.11 does not have** — drop the arg on crates.io 0.11.
- `Scene::draw_glyphs` takes `&FontData` in the clone (vello 0.9) but `&Font` in the vello_editor example (vello 0.8) — confirm the 0.9 signature.
- `CHROMIUM_LINE_BREAK_OVERRIDE` + `set_line_break_override` live in a `parley_core` split re-exported by `parley`; **confirm the published 0.11 point release actually exports them** (open question flagged across specs). If the crates.io 0.11 predates the `parley_core` split, this whole "browser-grade line breaking" goal is blocked until a version bump.

Phase 0 therefore begins with an **API reconciliation spike** to pin exact published versions and adapt every cited call site before any real porting starts.
...
**API spike required first** (see Strategy): stand up a throwaway `cargo` project pinning parley 0.11, vello 0.9, wgpu 29.0.3, winit 0.30.13; confirm `ranged_builder` arity (`quantize` present or not), `draw_glyphs(&FontData|&Font)`, and the presence of `CHROMIUM_LINE_BREAK_OVERRIDE` + `RangedBuilder::set_line_break_override` on the *published* surface. Record the exact adapted signatures; all later phases reference these, not the clone.
...
1. **API reconciliation spike:** new throwaway crate pinning parley 0.11 / vello 0.9 / wgpu 29.0.3 / winit 0.30.13; confirm `ranged_builder` arity (`quantize` present?), `Scene::draw_glyphs` signature, and that `CHROMIUM_LINE_BREAK_OVERRIDE` + `RangedBuilder::set_line_break_override` exist on the *published* surface. Write down the adapted signatures — every later phase cites these, not the clone.
